...
import { Braces, Code2, Pencil, Play, Save, TableProperties, Trash2, TriangleAlert } from "lucide-react";
...
export function QueryLabRoute({ initialModelName }: { initialModelName: string | null }) {
  ...
  const {
    isLeftCollapsed,
    isRightCollapsed,
    toggleLeft: toggleQueryLabControls,
    toggleRight: toggleQueryLabContext,
  } = useSidebarLayout("queryLab");
  ...
  return (
    <WorkspaceLayout
      activeRoute="query-lab"
      isLeftCollapsed={isLeftCollapsed}
      isRightCollapsed={isRightCollapsed}
      onToggleLeft={toggleQueryLabControls}
      onToggleRight={toggleQueryLabContext}
      leftCollapsedLabel="Show Query Lab controls"
      leftExpandedLabel="Hide Query Lab controls"
      rightCollapsedLabel="Show Query Lab context panel"
      rightExpandedLabel="Hide Query Lab context panel"
      leftSidebar={
        <WorkspaceSidebar side="left">
          <WorkspacePanelHeader
            title="Query Lab"
            icon={<Code2 className="h-4 w-4 text-primary" aria-hidden="true" />}
            actions={
              <SidebarToggleButton
                side="left"
                isCollapsed={false}
                collapsedLabel="Show Query Lab controls"
                expandedLabel="Hide Query Lab controls"
                onClick={toggleQueryLabControls}
              />
            }
          />
                      ...
                      >
                        <div className="flex items-start gap-1.5">
                          ...
                          <button
                            ...
                            title={`Rename saved Query Lab view ${view.name}`}
                          ...
                          <button
                            ...
                            title={`Delete saved Query Lab view ${view.name}`}
            ...
            <div className="min-h-[220px]">
              {queryLabResultViewModel.kind === "loading" ? (
                ...
                <div className="p-6">
                  ...
                  <CopyableCodeBlock
                    ariaLabel="Query Lab JSON result"
                    copyLabel="Copy Query Lab JSON result"
                    value={queryLabResultViewModel.json}
                    className="mt-3"
                    preClassName="bg-panel"
                  />
                </div>
              ) : queryLabResultViewModel.kind === "singleMiss" ? (
                <div className="p-6">
                ...
                <div>
                  ...
                  {resultMode === "table" ? (
                    ...
                    <CopyableCodeBlock
                      ariaLabel="Query Lab JSON result"
                      copyLabel="Copy Query Lab JSON result"
                      value={queryLabResultViewModel.resultJson}
                      className="m-3"
                      preClassName="max-h-none bg-panel"
                    />
                  )}
                </div>
              ) : queryLabResultViewModel.kind === "jsonOnly" ? (
        ...
        {!isRightCollapsed ? (
          <WorkspaceSidebar side="right">
            <WorkspacePanelHeader
              ...
              actions={
                <SidebarToggleButton
                  side="right"
                  isCollapsed={false}
                  collapsedLabel="Show Query Lab context panel"
                  expandedLabel="Hide Query Lab context panel"
                  onClick={toggleQueryLabContext}
                />
              }
            />
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-auto p-3">
              {queryLabResultViewModel.kind === "rows" ? (
                <section aria-label="Query Lab record preview">
                  <div className="mb-3">
              ...
              {queryInspector ? (
                <QueryInspectorPanel
                  ...
                  callFirst={false}
                  className={cn(
                    queryLabResultViewModel.kind === "rows" && "border-t border-border pt-3",
                  )}
                >
                        ...
                        <div aria-label="Query Lab SQL events" className="space-y-2">
                          {queryInspector.sqlEvents.map((event, index) => (
                            ...
                            >
                              ...
                              {event.query ? (
                                <CopyableCodeBlock
                                  ariaLabel={`Query Lab SQL ${index + 1}`}
                                  copyLabel={`Copy Query Lab SQL ${index + 1}`}
                                  value={event.query}
                                  preClassName="max-h-48 bg-panel p-2"
                                />
                              ) : (
                              ...
                              {event.params ? (
                                <CopyableCodeBlock
                                  ariaLabel={`Query Lab SQL params ${index + 1}`}
                                  copyLabel={`Copy Query Lab SQL params ${index + 1}`}
                                  value={event.params}
                                  className="mt-2"
                                  preClassName="max-h-32 bg-panel p-2"
                                />
                              ) : (
                ...
                <div className="rounded-md border border-dashed border-border bg-surface p-3 text-xs text-muted-foreground">
                  Run a Query Lab preview to inspect records and query details.
                </div>
              ) : null}
            </div>
          </WorkspaceSidebar>
